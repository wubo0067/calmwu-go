...
func ConvertObjToInterfaceMap(obj interface{}) (map[string]interface{}, error) {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		utils.ZLog.Debugf("fileName[%s] type:%v value:%v valueType:%v kind:%v\n",
			fieldName, field.Type, fieldValue, fieldValue.Type(), field.Type.Kind())
		switch field.Type.Kind() {
		case reflect.String:
			redisHash[fieldName] = fieldValue.String()
	...
	if v.Kind() == reflect.Ptr {
		...
		for i < t.NumField() {
			...
			if redisV, ok := hashV[fieldName]; !ok {
				utils.ZLog.Warnf("field[%s] value is not exists!", fieldName)
				i++
				continue
			} else {
				if v.Elem().Field(i).CanSet() {
					...
					case reflect.Struct:
						...
						if field.Type.String() == "time.Time" {
							timeV, err := time.Parse("2006-01-02 15:04:05 -0700", redisV)
							if err != nil {
								return err
							}
