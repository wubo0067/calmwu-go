...
FAILED:
	utils.ZLog.Errorf("UnPackProtoHead failed! reason[%s]", err.Error())
...
func PackSynMsg(msgId uint32, protoSyn *ProtoSyn) ([]byte, error) {
	...
	if err != nil {
		utils.ZLog.Errorf("PackSynMsg failed! reason[%s]", err.Error())
...
func PackAsynMsg(msgId uint32, protoAsyn *ProtoAsyn) ([]byte, error) {
	if protoAsyn == nil {
		return nil, errors.New("input args is invalid!")
	}
	serialBuf, err := proto.Marshal(protoAsyn)
	if err != nil {
		utils.ZLog.Errorf("PackAsynMsg failed! reason[%s]", err.Error())
		return nil, ErrPackProtoSyn
	}
...
func PackAckMsg(msgId uint32, protoAck *ProtoAck) ([]byte, error) {
	...
	if err != nil {
		utils.ZLog.Errorf("PackAckMsg failed! reason[%s]", err.Error())
		return nil, ErrPackProtoSyn
	}
