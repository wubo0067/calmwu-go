 ...
 * @Last Modified time: 2017-12-25 15:50:09
 * @Comment:
 */
	...
	for {
		...
		if err != nil {
			...
			if err != nil {
				...
				case CMD_STOP_REQ:
					base.GLog.Info("GuideSvr receive CMD_STOP_REQ")
					os.Exit(1)
				case CMD_RELOAD_REQ:
					base.GLog.Info("GuideSvr receive CMD_RELOAD_REQ")
					var reloadResCmd ControlCmdS
					reloadResCmd.CmdName = CMD_RELOAD_RES
					reloadResCmd.CmdData = fmt.Sprintf("GuildSvr[%s] result[%s]",
	...
	if err != nil {
		...
		for _, ip := range ipLst {
			for _, portName := range portLst {
				...
				if err == nil {
					fmt.Printf("GuideSvr[%s:%d] ReloadReq\n", ip, port)
					res := sendControlCmd(ip, port, realCmd)
					fmt.Printf("GuideSvr[%s:%d] ReloadRes[%s]\n", ip, port, res)
				} else {
