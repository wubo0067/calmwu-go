 ...
 * @Last Modified time: 2017-11-24 15:01:07
 * @Comment:
 */
...
func ConsulGlobalSeq(client *api.Client, seqName string, initSeqNum int, waitTime string) (int, error) {
	...
	if err != nil {
		base.GLog.Error("SeqName[%s] parse waitTime failed! reason[%s]", seqName, err.Error())
		return 0, err
	}
	...
	if err != nil {
		base.GLog.Error("SeqName[%s] session create failed! reason[%s]", seqName, err.Error())
		return 0, err
	}
	...
	for {
		...
		if elapsed > lockWaitTime {
			// 等待超时
			base.GLog.Warn("SeqName[%s] wait timeout!", seqName)
		...
		if err != nil {
			base.GLog.Error("SeqName[%s] get failed! reason[%s]", seqName, err.Error())
			return 0, err
		}
		...
		if err != nil {
			base.GLog.Error("SeqName[%s] acquire failed! reason[%s]", seqName, err.Error())
			return 0, err
		}
