...
func ZlibCompress(data []byte, compressLevel int) ([]byte, error) {
	...
	if err != nil {
		GLog.Error("zlib.NewWriterLevel failed! reason[%s]", err.Error())
		return nil, err
	}

	_, err = compress.Write(data)
	if err != nil {
		GLog.Error("zlib.Writer failed! reason[%s]", err.Error())
		return nil, err
	}

	err = compress.Close()
	if err != nil {
		GLog.Error("zlib.Close failed! reason[%s]", err.Error())
		return nil, err
	}
