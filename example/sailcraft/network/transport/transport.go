...
func StartNetTransport(listenIP string, listenPort int, config *NetTransportConfig) (*NetTransport, error) {
	...
	if err != nil {
		base.GLog.Error("Failed to start TCP listener on %q port %d. reason[%s]",
			listenIP, listenPort, err.Error())
		return nil, err
	}

	base.GLog.Debug("NetTransport confiig:%+v", *config)
	...
	base.GLog.Info("NetTransport started! listen on[%s:%d]", listenIP, listenPort)
	return netTransport, nil
}

func (nt *NetTransport) ShutDown() {
	defer base.GLog.Info("NetTransport ShutDown!!")
...
func (nt *NetTransport) tcpListenerRoutine() {
	defer base.GLog.Info("tcpListenerRoutine exit!")
	defer nt.wg.Done()
	...
	for {
		...
		if err != nil {
			if s := atomic.LoadInt32(&nt.shutdown); s == 1 {
				base.GLog.Warn("tcpListenerRoutine shutdown flag is set!")
				break
			}

			base.GLog.Error("Error accepting Tcp connection! reason[%s]", err.Error())
			continue
		}

		if nt.CurrSessionCount() < nt.config.maxSessionCount {
			nt.connCh <- conn
			base.GLog.Debug("Accept new connect[%s]", conn.RemoteAddr().String())
		} else {
			conn.Close()
			base.GLog.Error("The session count exceeds the limit!")
		}
	}
}

func (nt *NetTransport) tcpNewConnectRoutine() {
	defer base.GLog.Info("tcpNewConnectRoutine exit!")
	defer nt.wg.Done()
	...
	for {
		...
		case <-ticker.C:
			base.GLog.Debug("Current Session Count:%d", nt.CurrSessionCount())
		case <-nt.shutdownCh:
			base.GLog.Warn("tcpNewConnectRoutine receive shutdown notify")
			return
		}
	}
}
...
func (nt *NetTransport) removeSession(sessionID uint32) {
	...
	if _, ok := nt.sessionMap[sessionID]; ok {
		...
		base.GLog.Error("sessionID[%d] is not exist!", sessionID)
	}
}
...
func (nt *NetTransport) dispatchSessionData(sessionData *NetSessionData) {
	...
	if session, ok := nt.sessionMap[sessionData.SessionID]; ok {
		...
		base.GLog.Error("Session[%d] can not find in sessionMap!", sessionData.SessionID)
	}
}
