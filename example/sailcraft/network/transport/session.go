...
func NewSession(sessionID uint32, conn *net.TCPConn, nt *NetTransport) *NetSession {
	...
	base.GLog.Debug("Create new session for %s", newSession.RemoteAddr())
...
func (ns *NetSession) handleConn() {
	base.GLog.Info("Session[%d] New incoming connection[%s]", ns.sessionID, ns.conn.RemoteAddr().String())
	...
	defer func() {
		...
		if err := recover(); err != nil {
			// 回收panic，防止整个服务panic
			base.GLog.Error("Session[%d] painc! reson:%v, stack:%s", ns.sessionID, err, base.GetCallStack())
		}
		base.GLog.Debug("Session[%d] disconnect with client[%s]", ns.sessionID, ns.RemoteAddr())
	}()
	...
	base.SetRecvBuf(ns.conn, DEF_TCPRECVBUF_SIZE)

	for {
		if s := atomic.LoadInt32(&ns.shutdown); s == 1 {
			base.GLog.Warn("Session[%d] Shutdown flag is set!", ns.sessionID)
			return
		}
		// 读取头
		...
		if err != nil {
			// 错误处理
			if err == io.EOF {
				base.GLog.Warn("Session[%d] client[%s] disconnect the session. reason[%s]",
				...
				if oerr.Timeout() && time.Since(readDataTime) >= ns.nt.config.sessionIdleTimeout {
					// read timeout
					base.GLog.Warn("Session[%d] idle exceed limit readDataTime[%s] now[%s], so will disconnect!",
						ns.sessionID, base.TimeName(readDataTime), base.GetTimeStampSec())
					return
					...
					// read RST
					base.GLog.Warn("Session[%d] receive RESET, so will disconnect! SourceAddr[%s] Addr[%s] reason[%s]",
						ns.sessionID, oerr.Source.String(), oerr.Addr.String(), err.Error())
					return
				}
			} else {
				// peekSize > reader的缓冲区，会返回错误ErrBufferFull
				base.GLog.Error("Session[%d] err:%s buffSize:%d", ns.sessionID, err.Error(), ns.reader.Buffered())
				return
			}
		} else {
			//数据处理
			...
			case E_SESSIONPEEKSTATE_HEAD:
				...
				if err != nil {
					base.GLog.Error("Session[%d] UnPackProtoHead failed so will disconnect. reason[%s]", ns.sessionID, err.Error())
					return
				} else {
					// 对头进行校验
					if protoHead.MagicValue != protocol.ProtoC2SMagic {
						base.GLog.Error("Session[%d] check head magic failed so will disconnect", ns.sessionID)
						return
					}

					// 判断包大小的合法性
					if protoHead.DataLen < 0 || int(protoHead.DataLen) > ns.nt.config.netPkgMaxSize {
						base.GLog.Error("Session[%d] check head datalen[%d] is illegal", ns.sessionID, protoHead.DataLen)
						return
					}

					//base.GLog.Debug("head:%+v", *protoHead)
					peekSize = protocol.ProtoC2SHeadSize + int(protoHead.DataLen)
					peekState = E_SESSIONPEEKSTATE_WHOLE
					//base.GLog.Debug("Session[%d] package whole size[%d] protoHead:%+v", ns.sessionID, peekSize, *protoHead)
				}
			case E_SESSIONPEEKSTATE_WHOLE:
				if ns.handleNetPkg(protoHead, data[protocol.ProtoC2SHeadSize:]) < 0 {
					base.GLog.Error("Session[%d] handleNetPkg failed so will disconnect", ns.sessionID)
					return
				}
...
func (ns *NetSession) handleBusiness() {
	...
	for {
		...
		case sessionData := <-ns.frombussinessCh:
			...
			case E_SESSIONCMD_STOPCONN:
				...
				if sessionData.SessionID == ns.sessionID && ns.sessionState == E_SESSIONSTATE_CONNECTED {
					base.GLog.Info("Session[%d] recv E_SESSIONCMD_STOPCONN from business, now will disconnect session!",
						ns.sessionID)
				} else {
					base.GLog.Info("Session[%d] recv E_SESSIONCMD_STOPCONN, but sessionid[%d] is mismatching",
						ns.sessionID)
				}
			case E_SESSIONCMD_TRANSFER:
				if sessionData.SessionID == ns.sessionID && ns.sessionState == E_SESSIONSTATE_CONNECTED {
					...
					if err != nil {
						base.GLog.Error("Session[%d] transfer data FlodPayloadData failed! reason[%s]",
		...
		case <-ns.nt.shutdownCh:
			base.GLog.Info("Session[%d] handleBusiness receive shutdown notify!", ns.sessionID)
...
func (ns *NetSession) handleNetPkg(head *protocol.ProtoC2SHead, data []byte) int {
	...
	case E_SESSIONSTATE_VERIFIY_SYN:
		...
		if head.Cmd == protocol.E_C2S_CMD_SYN {
			...
			if err != nil {
				base.GLog.Error("Session[%d] unmarshal ProtoSyn failed! reason[%s]",
					ns.sessionID, err.Error())
				return -1
			}

			base.GLog.Debug("Session[%d] synPkg:%+v", ns.sessionID, syn)

			dhKey, err := base.GenerateDHKey()
			if err != nil {
				base.GLog.Error("Session[%d] GenerateDHKey failed! reason[%s]",
					ns.sessionID, err.Error())
				return -1
			}

			// 根据对方发过来的ka，计算自己的秘钥串
			key, err := base.GenerateEncryptionKey(syn.DHClientPubKey, dhKey)
			if err != nil {
				base.GLog.Error("Session[%d] GenerateEncryptionKey failed! reason[%s]",
					ns.sessionID, err.Error())
				return -1
			}

			secretKeyB := hex.EncodeToString(key)
			base.GLog.Debug("golang server secretKeyB[%s]", secretKeyB)

			// aes-256
			ns.encryptionKey, err = base.NewCipherBlock(key[:32])
			if err != nil {
				base.GLog.Error("Session[%d] NewCipherBlock failed! reason[%s]",
					ns.sessionID, err.Error())
				return -1
			}
			...
			if err != nil {
				base.GLog.Error("Session[%d] PackAsynMsg failed! reason[%s]",
					ns.sessionID, err.Error())
				return -1
			}
			...
			base.GLog.Error("Session[%d] currState[%s] receive Cmd[%s] is invalid!", ns.sessionID,
	...
	case E_SESSIONSTATE_VERIFIY_ACK:
		...
		if head.Cmd == protocol.E_C2S_CMD_ACK {
			...
			if err != nil {
				base.GLog.Error("Session[%d] Unmarshal ProtoAck failed! reason[%s]", ns.sessionID, err.Error())
				return -1
			}
			// 对校验数据进行加密，比较
			myCipherText, err := base.EncryptPlainText(ns.encryptionKey, ns.verifyBuf)
			if err != nil {
				base.GLog.Error("Session[%d] EncryptPlainText failed! reason[%s]", ns.sessionID,
					err.Error())
				return -1
			}
			if bytes.Equal(ack.CipherText, myCipherText) {
				// 握手成功
				base.GLog.Info("Session[%d] remote[%s] handshake ok!", ns.sessionID, ns.RemoteAddr())
				ns.notifyBusinessConnStart()
				ns.sessionState = E_SESSIONSTATE_CONNECTED
			} else {
				base.GLog.Error("Session[%d] encrypt check failed! CipherText:%v, myCipherText:%v",
	...
	case E_SESSIONSTATE_CONNECTED:
		...
		if head.Cmd == protocol.E_TRANSFER {
			...
			if err != nil {
				base.GLog.Error("Session[%d] UnFlodPayloadData failed! reason[%s]", ns.sessionID, err.Error())
			} else {
				// 透传数据
			...
			if err != nil {
				base.GLog.Error("Session[%d] Unmarshal ProtoHeartBeat failed! reason[%s]", ns.sessionID, err.Error())
				return -1
			}
...
func (ns *NetSession) handleHeartBeat(msgId uint32, unix int64) {
	// 回应
	//base.GLog.Debug("handle remote[%s] heartbeat", ns.RemoteAddr())
...
func (ns *NetSession) notifyBusinessConnStart() {
	...
	base.GLog.Debug("Session[%d] send [%s] to bussiness", ns.sessionID, ns.sessionState.String())
	return
}
...
func (ns *NetSession) notfityBusinessConnStop() {
	if ns.sessionState == E_SESSIONSTATE_CONNECTED {
		...
		base.GLog.Warn("Session[%d] currState[%s] no need to send cmd[E_SESSIONSTATE_CONNECTED]",
