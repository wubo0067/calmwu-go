...
func startSession() {
	...
	key, err := base.GenerateEncryptionKey(synAck.DHServerPubKey, dhKey)
	if err != nil {
		fmt.Println(err.Error())
		return
	}
	...
	cipherBlock, err := base.NewCipherBlock(key[:32])
	if err != nil {
		fmt.Println(err.Error())
		return
	}
	...
	// 加密
	cipherBuf, err := base.EncryptPlainText(cipherBlock, []byte("123456qwaszx"))
	if err != nil {
		fmt.Println(err.Error())
		return
	}
