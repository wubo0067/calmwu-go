...
func FlodPayloadData(rawData []byte, cipherBlock cipher.Block) ([]byte, error) {
	...
	// 压缩
	cData, err := base.ZlibCompress(rawData, zlib.BestCompression)
	if err != nil {
		return nil, err
	}
	...
	// 加密
	cipherBuf, err := base.EncryptPlainText(cipherBlock, crcBuf)
	if err != nil {
		return nil, err
	}
...
func UnFlodPayloadData(cipherData []byte, cipherBlock cipher.Block) ([]byte, error) {
	...
	// 解密
	crcBuf, err := base.DecryptCipherText(cipherBlock, cipherData)
	if err != nil {
		return nil, err
	}
	//fmt.Printf("crcBuf len:%d\n", len(crcBuf))
	...
	// 解压
	rawText, err := base.ZlibDCompress(crcBuf[8:crcBufLen])
	if err != nil {
		return nil, err
	}
	return rawText, nil
