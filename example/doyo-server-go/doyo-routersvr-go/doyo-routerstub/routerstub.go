 ...
 * @Last Modified time: 2018-11-01 19:11:28
 */
...
func NewRouterStubModule(svrName string, svrInstName string, doyoKfk *doyokafka.KafkaModule, onReceive OnReceive,
	consulSvrAddr string, healthCheckAddr string, healthCheckPort int) (*RouterStubModule, error) {
	rs := &RouterStubModule{
		doyoKfk:              doyoKfk,
	...
	httpCheckUrl := fmt.Sprintf("http://%s:%d%s", healthCheckAddr, healthCheckPort, checkUrl)
	...
	go func() {
		...
		http.ListenAndServe(fmt.Sprintf("%s:%d", healthCheckAddr, healthCheckPort), nil)
	}()

	// 注册
	ConsulClient, err := consul_api.NewConsulClient(consulSvrAddr)
	if err != nil {
		base.ZLog.Errorf("NewConsulClient failed! reason[%s]", err.Error())
		return nil, err
	}

	err = consul_api.ConsulSvrReg(ConsulClient, svrName, []string{svrName}, svrInstName, healthCheckAddr,
		healthCheckPort, httpCheckUrl)
	if err != nil {
