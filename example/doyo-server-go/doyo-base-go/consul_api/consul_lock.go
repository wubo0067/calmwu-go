...
func ConsulGlobalLock(client *api.Client, lockName string, watiLockTime string) (*api.Lock, error) {
	...
	base.ZLog.Debugf("lockName[%s] watiLockTime[%s]", lockName, watiLockTime)

	// 加锁
	leaderCh, err := lock.Lock(nil)
	if err != nil {
		base.ZLog.Errorf("Lock %s failed! reason[%s]", lockName, err.Error())
		return nil, err
	}

	if leaderCh == nil && err == nil {
		base.ZLog.Errorf("Lock %s acquire timeout", lockName)
	...
	go func() {
		select {
		case <-leaderCh:
			base.ZLog.Warnf("Lock %s not leader, must be check!!!")
		}
	}()
...
func ReleaseGlobalLock(lock *api.Lock) error {
	...
	err := lock.Unlock()
	if err != nil {
		base.ZLog.Errorf("Lock release failed! reason[%s]", err.Error())
	}

	err = lock.Destroy()
	if err != nil {
		base.ZLog.Errorf("Lock destroy failed! reason[%s]", err.Error())
	}
