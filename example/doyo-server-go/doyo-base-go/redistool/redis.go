...
func (rm *RedisMgr) Start() error {
	base.ZLog.Debugf("redisSvrAddrs:%v", rm.redisSvrAddrs)
	...
	if rm.isCluster {
		rm.redisClusterClient = redis.NewClusterClient(&redis.ClusterOptions{
			...
			OnConnect: func(conn *redis.Conn) error {
				base.ZLog.Infof("redis.Conn:%s", conn.String())
				return nil
			},
	...
	if rm.isCluster {
		...
		base.ZLog.Debugf("redisClusterClient Ping:%s", res)
	} else {
		res, err = rm.redisClient.Ping().Result()
		base.ZLog.Debugf("redisClient Ping:%s", res)
	}
...
func redisCmdPerformRoutine(rm *RedisMgr) {
	base.ZLog.Debug("redisCmdPerformRoutine running")
	...
	defer func() {
		...
		if err := recover(); err != nil {
			...
			base.ZLog.Warnw("redisCmdPerformRoutine panic recovered! err", err, "stack", string(stackInfo))
		}
	}()
L:
	for {
		...
		case redisCmdData, ok := <-rm.redisCmdChan:
			...
			if ok {
				...
				case REDIS_GET:
					...
					if err != nil {
						base.ZLog.Errorf("Get key[%s] failed! error:%s", redisCmdData.key, err.Error())
				...
				case REDIS_SET:
					...
					if err == nil {
						...
						base.ZLog.Errorf("Set key[%s] failed! error:%s", redisCmdData.key, err.Error())
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					}
				case REDIS_DEL:
					res, err := rm.redisCmdable.Del(redisCmdData.key).Result()
					base.ZLog.Debugf("Del key[%s] res[%d]", redisCmdData.key, res)
					if err != nil {
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					} else {
				...
				case REDIS_SETNX:
					...
					if err == nil {
						...
						base.ZLog.Errorf("SetNX key[%s] failed! error:%s", redisCmdData.key, err.Error())
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					}
				case REDIS_HGETALL:
					...
					if err == nil {
						...
						base.ZLog.Errorf("HGETALL key[%s] failed! error:%s", redisCmdData.key, err.Error())
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					}
				case REDIS_HMGET:
				case REDIS_HMSET:
					res, err := rm.redisCmdable.HMSet(redisCmdData.key, redisCmdData.value.(map[string]interface{})).Result()
					base.ZLog.Debugf("HMSET key[%s] res:%v", redisCmdData.key, res)
					if err == nil {
						// set成功
						redisCmdData.replyChan <- &RedisResultS{Ok: true, Result: res}
					} else {
						base.ZLog.Errorf("HMSET key[%s] failed! error:%s", redisCmdData.key, err.Error())
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					}
				case REDIS_HSET:
					res, err := rm.redisCmdable.HSet(redisCmdData.key, redisCmdData.fieldKey, redisCmdData.value).Result()
					base.ZLog.Debugf("HSET key[%s] fieldkey[%s] res:%v", redisCmdData.key, redisCmdData.fieldKey, res)
					if err == nil {
						// set成功
						redisCmdData.replyChan <- &RedisResultS{Ok: true, Result: res}
					} else {
						base.ZLog.Errorf("HSET key[%s] fieldKey[%s] failed! error:%s", redisCmdData.key,
				...
				case REDIS_PIPELINE:
					...
					if err != nil {
						base.ZLog.Error("Pipelined failed! reason:%s", err.Error())
					}
					if cmds != nil && len(cmds) > 0 {
						base.ZLog.Debugf("Pipelined cmds:%v", cmds)
					}
				...
				case REDIS_ZINCRBY:
					...
					base.ZLog.Debugf("ZIncrBy key[%s] increment[%v] memberKey[%s] val:%v", redisCmdData.key, redisCmdData.value,
				...
				case REDIS_EXPIRE:
					...
					base.ZLog.Debugf("Expire key[%s] expiration:%v", redisCmdData.key, redisCmdData.value)
					if err != nil {
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					} else {
						redisCmdData.replyChan <- &RedisResultS{Ok: true, Result: res}
				...
				case REDIS_DBSIZE:
					...
					base.ZLog.Debugf("DBSize count:%d", res)
					if err != nil {
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					} else {
						redisCmdData.replyChan <- &RedisResultS{Ok: true, Result: res}
				...
				case REDIS_SCAN:
					...
					base.ZLog.Debugf("Scan keycount[%d] cursor[%d]", len(scanRes.keys), scanRes.cursor)
					if err != nil {
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					} else {
				...
				case REDIS_CLUSTERSCAN:
					...
					base.ZLog.Debugf("Scan keycount[%d] cursor[%d] err:%v", len(scanRes.keys), scanRes.cursor, err)
					if err != nil {
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					} else {
						redisCmdData.replyChan <- &RedisResultS{Ok: true, Result: scanRes}
				...
				case REDIS_SADD:
					...
					base.ZLog.Debugf("SADD key[%s] res:%d", redisCmdData.key, res)
					if err != nil {
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					} else {
						redisCmdData.replyChan <- &RedisResultS{Ok: true, Result: res}
				...
				case REDIS_SREM:
					...
					base.ZLog.Debugf("SREM key[%s] res:%d", redisCmdData.key, res)
					if err != nil {
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					} else {
						redisCmdData.replyChan <- &RedisResultS{Ok: true, Result: res}
				...
				case REDIS_SISMEMBER:
					...
					base.ZLog.Debugf("SISMEMBER key[%s] res:%v", redisCmdData.key, res)
					if err != nil {
						redisCmdData.replyChan <- &RedisResultS{Ok: false, Result: err}
					} else {
						redisCmdData.replyChan <- &RedisResultS{Ok: true, Result: res}
	...
	base.ZLog.Warnf("redisCmdPerformRoutine Exit")
	return
}
